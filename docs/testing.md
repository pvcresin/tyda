...
Rust build / clippy / test（Windowsも全test targetを実行し、`scripts/ci/run-tests-parallel.sh` で一度だけコンパイルしてから独立したテストプロセスを並列実行する）、`Performance` は pinned な Ruby / Rails OSS subject ごとの base/head 比較、release workflowはgem / VSIXのLinux x86_64・Windows x64・Intel macOS・
